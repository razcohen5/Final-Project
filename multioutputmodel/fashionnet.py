...
class FashionNet:
    ...
    def build_size_branch(inputs, numSizes, finalAct="sigmoid",
		chanDim=-1):
        # CONV => RELU => POOL
        ...
        x = Dropout(0.5)(x)
        x = Dense(numSizes)(x)
        x = Activation(finalAct, name="size_output")(x)
    ...
    def build(width, height, numSizes, numCategories, numColors,
		finalAct="softmax"):
        ...
        sizeBranch = FashionNet.build_size_branch(inputs,
			numSizes, chanDim=chanDim)
        ...
        model = Model(
			inputs=inputs,
			outputs=[sizeBranch, categoryBranch, colorBranch],
			name="fashionnet")
